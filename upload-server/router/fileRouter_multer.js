...
router.post('/test', (req, res) => {
    // fs.chmodSync(path.join(req.files.file.path), 755)
    console.log(req.files, '\n', req.body); 
    console.log('__dirname', __dirname); // 返回当前模块的目录名
    console.log('process.cwd()', process.cwd()); // 当前Node.js进程执行时的文件夹地址——工作目录，保证了文件在不同的目录下执行时，路径始终不变
    console.log('__filename', __filename); // 返回当前模块的文件名
    console.log('path.dirname', path.dirname(__filename)); // __dirname 等同于 path.dirname(__filename)
    res.send({
        code: 200,
        codeText: '响应成功!',
    });
...
router.post('/upload_single', async (req, res) => {
  ...
  for (let file of files) {//将files循环成单个
    ...
    //   file_name = req.body.filename.split('.')[0] + '[' + createRandom() + ']' + '.' + file_ext;  // 将文件名改为 文件名[随机数唯一值].xxx 格式
      // 移动文件并且修改文件名字
      fs.renameSync(
        path.join(process.cwd(), "/public/upload/temp/", file.filename),//file.filename：文件最初名字
        path.join(process.cwd(), "/public/upload/", file_name), //file_name：时间戳新起的名字
      );
    //   fs.chmodSync(path.join(process.cwd(), "/public/upload/", file_name), 755)
      //将改完的文件写进空数组
      ret_files.push(path.join(process.cwd(), "/public/upload/", file_name));
  }
    ...
    // 移动文件并且修改文件名字
    // fs.renameSync(
    //     files.path,//file.filename：文件最初名字
    //     path.join(process.cwd(), "/public/upload/", files.originalFilename), //file_name：时间戳新起的名字
    // );
    // fs.chmodSync(path.join(process.cwd(), "/public/upload/", files.originalFilename), 755)
    // //将改完的文件写进空数组
    // ret_files.push(path.join(process.cwd(), "/public/upload/", files.originalFilename));
...
//   console.log(files.originalFilename, '文件上传成功！' + '\n' + '名称修改为：', file_name ? file_name : req.body.filename, '\n--------------------');
});
...
router.post('/upload_single_base64', async (req, res) => {
    ...
    file_path = path.join(`${uploadDir}/${spark.end()}.${suffix}`);  // （通过spark.end()拿到生成的hash名字）
    // await delay();  // 延时函数
    // 检测是否存在
    isExists = await exists(file_path);  // 通过保存路径判断文件是否存在（已上传过）
    if (isExists) {
        res.send({
            code: 200,
            ...
            originalFilename: filename,
            // servicePath: path.replace(__dirname, HOSTNAME)
            servicePath: file_path
        });
        console.log(req.body.filename, '文件已存在！' + '\n' + '名称为：', `${spark.end()}.${suffix}`, '\n--------------------');
        return;
    };
    writeFile(res, file_path, file, filename, false);  // 写入目录
...
router.post('/upload_single_name', async (req, res) => {
    ...
    //保存文件
    let files = req.files; //将获取的文件放到files
    let file_path = path.join(process.cwd(), "/public/upload/", req.body.filename);  // 路径
    ...
    for (let file of files) {//将files循环成单个
        ...
        if (isExists) {
            ...
            res.send({
                code: 200,
                codeText: '文件上传成功！',
                servicePath: file_path //返回data给前端预览
            })
        ...
        fs.renameSync(
            path.join(process.cwd(), "/public/upload/temp/" + file.filename),//file.filename：文件最初名字
            path.join(process.cwd(), "/public/upload/", filename), //file_name：新起的名字
        );
    }
});
...
const merge = function merge(HASH, count) {
    return new Promise(async (resolve, reject) => {
        let file_path = path.join(uploadDir, HASH),
            fileList = [],
            suffix,
            isExists;
        isExists = await exists(file_path);
        // console.log('merge', file_path, isExists)
        if(!isExists) {
            reject('HASH path is not found!');
            return;
        }
        fileList = fs.readdirSync(file_path);
        console.log('merge', isExists, fileList.length, count)
        ...
        fileList.sort((a, b) => {
            ...
            fs.appendFileSync(path.join(uploadDir, `${HASH}.${suffix}`), fs.readFileSync(path.join(file_path, item)));
            fs.unlinkSync(path.join(file_path, item));
        })
...
router.post('/upload_chunk', async (req, res) => {
    ...
    try {
        ...
        // 创建存放切片的临时目录
        let [, HASH] = /^([^_]+)_(\d)/.exec(filename);
        file_path = path.join(`${uploadDir}/${HASH}`);
        !fs.existsSync(file_path) ? fs.mkdirSync(file_path) : null;
        // 把切片存储到临时目录中
        file_path = path.join(`${uploadDir}/${HASH}/${filename}`);
        // console.log(path)
        isExists = await exists(file_path);
        if (isExists) {
            res.send({
        ...
        // console.log('写入目录----------')
        writeFile(res, file_path, file, filename, true);  // 写入目录
...
router.post('/upload_merge', async (req, res) => {
    ...
    try {
        ...
        // console.log('upload_merge', filename, filepath)
        res.send({
            code: 200,
