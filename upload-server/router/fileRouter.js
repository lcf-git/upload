...
//   console.log(req.files, req.body)
  ...
  //保存文件
  let files = req.files.file; //将获取的文件放到files
...
//       // 移动文件并且修改文件名字
//       fs.renameSync(
//         path.join(process.cwd(), "\\public\\upload\\temp\\", file.filename),//file.filename：文件最初名字
...
//   }
    ...
    // file_ext = file.originalFilename.substring(file.originalFilename.lastIndexOf('.') + 1);  // 获取名字后缀
    // file_name = req.body.filename.split('.')[0] + '[' + new Date().getTime() + ']' + '.' + file_ext;  // 将文件名改为 文件名[时间戳].xxx 格式
    // 移动文件并且修改文件名字
    fs.renameSync(
